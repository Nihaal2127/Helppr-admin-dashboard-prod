...
- **`src/pages` is `.tsx` only**: Route screens and dialogs live here. Put supporting `.ts` in **`src/quote/`** (quote UI logic), **`src/helper/<module>/`**, or **`src/mockData/<module>/`** as appropriate—see `quote/`, `helper/reports/`, `helper/ticketManagement/`, `mockData/ticketManagement/`.
